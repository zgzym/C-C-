...
```
	...
	Opencv中矩阵的拷贝分为浅拷贝和深拷贝，浅拷贝为
```c++
	Mat B = A;
	Mat B(A);
```
	浅拷贝情况下，B和A共享同一内存空间，因此B可以看作A的引用，改变B会同时改变A。
...
```c++
	...
	B = A.clone();
```
	.clone()为完全深拷贝，即B和A必定享有独立的内存空间；
	
	而.copyTo()则分2中情况，第1种情况，B和A的矩阵大小一致时，不会开辟新的内存空间；
	
	第2种情况，B和A的矩阵大小不一致时，则会开辟新的内存空间。
